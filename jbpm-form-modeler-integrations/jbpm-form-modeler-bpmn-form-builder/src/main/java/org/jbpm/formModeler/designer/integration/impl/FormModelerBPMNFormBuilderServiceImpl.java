...
@Dependent
public class FormModelerBPMNFormBuilderServiceImpl implements BPMNFormBuilderService<Definitions> {
    ...
    @Inject
    public FormModelerBPMNFormBuilderServiceImpl( @Named( "ioStrategy" ) IOService ioService,
        ...
        this.dataHolderManager = dataHolderManager;
    }
    ...
    @Override
    public String buildFormContent( Path formPath, Definitions source, String id ) throws Exception {
        ...
        if ( !ioService.exists( kiePath ) ) {
            form = formManager.createForm( formPath.getFileName() );
            ...
            if ( re instanceof org.eclipse.bpmn2.Process ) {
                ...
                if ( process != null && process.getId() != null && process.getId().length() > 0 ) {
                    ...
                    if ( StringUtils.isEmpty( resourceId ) )
                        return getProcessDataHolders( processProperties, contextUri );

                    String[] colors = dataHolderManager.getHolderColors().keySet().toArray( new String[0] );
                    int index = 0;

                    for ( Property prop : processProperties ) {
                        String holderClass = StringUtils.defaultIfEmpty( prop.getItemSubjectRef().getStructureRef(),
                                                                         "java.lang.Object" );
                        DataHolderBuildConfig config = new DataHolderBuildConfig( prop.getId(),
                                                                                  "",
                                                                                  "",
                                                                                  colors[index],
                                                                                  holderClass );
    ...
    private void getDataHoldersFromElements( FlowElementsContainer container,
                                             String resourceId,
                                             Map<String, DataHolderBuildConfig> associations ) {
                ...
                if ( dataInputAssociations != null ) {
                    for ( DataInputAssociation inputAssociation : dataInputAssociations ) {
                        ...
                        if ( inputAssociation.getSourceRef() != null && inputAssociation.getSourceRef().size() > 0 && inputAssociation.getTargetRef() != null ) {
                            ...
                            DataHolderBuildConfig config = ( ( variableId != null ) && ( id != null ) ) ? associations.get(
                                    variableId ) : null;
                ...
                if ( dataOutputAssociations != null ) {
                    for ( DataOutputAssociation outputAssociation : dataOutputAssociations ) {
                        ...
                        if ( outputAssociation.getSourceRef() != null && outputAssociation.getSourceRef().size() > 0 && outputAssociation.getTargetRef() != null ) {
                            ...
                            DataHolderBuildConfig config = ( ( variableId != null ) && ( outId != null ) ) ? associations.get(
                                    variableId ) : null;
    ...
    private Set<DataHolder> getProcessDataHolders( List<Property> processProperties, String path ) {
        ...
        for ( Property prop : processProperties ) {
            ...
            String propertyType = StringUtils.defaultIfEmpty( prop.getItemSubjectRef().getStructureRef(),
                                                              "java.lang.Object" );

            DataHolderBuildConfig config = new DataHolderBuildConfig( propertyName,
                                                                      "",
                                                                      propertyName,
                                                                      colors[index],
                                                                      propertyType );
    ...
    private DataHolder createDataHolder( DataHolderBuildConfig config ) {
        ...
        DataHolderBuilder builder = dataHolderManager.getBuilderByHolderValueType( type,
