...
public class LinkFieldHandler extends DefaultFieldHandler {
    ...
    public Object getValue(Field field, String inputName, Map parametersMap, Map filesMap, String desiredClassName, Object previousValue) throws Exception {
        String[] nameValue = (String[]) parametersMap.get(inputName + "_name");
        String[] linkValue = (String[]) parametersMap.get(inputName + "_link");

        if (linkValue == null || linkValue.length == 0) return null;
        if (nameValue == null || nameValue.length == 0) return null;

        if ("".equals(linkValue[0]) && !"".equals(nameValue[0]))
            throw new IllegalArgumentException("No link data");

        return new Link(linkValue[0], nameValue[0]);
    ...
    public Map getParamValue(String inputName, Object objectValue, String pattern) {
        ...
        Map m = new HashMap();
        m.put(inputName + "_name", new String[]{((Link) objectValue).getName()});
        m.put(inputName + "_link", new String[]{((Link) objectValue).getLink()});
        return m;
    }
