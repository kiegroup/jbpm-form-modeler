...
import org.jbpm.formModeler.service.bb.commons.io.DirectoriesScanner;
...
public final class Factory {
    ...
    protected List<String> processedJars = null;
    ...
    protected List<DescriptorFile> initModuleFromJar(String moduleName) throws IOException {
        List<DescriptorFile> descriptorFiles = new ArrayList<DescriptorFile>();
        String libPath = ConfigurationManager.singleton().getBaseAppDirectory() + File.separator + "WEB-INF" + File.separator + "lib";
        File libDir = new File(libPath);
        DirectoriesScanner ds = new DirectoriesScanner("jar");
        File[] jars = ds.findFiles(libDir);
        for (int i = 0; i < jars.length; i++) {
            File jar = jars[i];
            if (jar.getName().startsWith(moduleName) && !processedJars.contains(jar.getName())) {
                processedJars.add(jar.getName());
    ...
    public List<String> getProcessedJars() {
    ...
    public static Object lookup(String path) {
        Factory factory = ConfigurationManager.singleton().getGlobalFactory();
        if (factory == null) return null;
        return factory.doLookup(path, null);
    }

    public static Object lookup(String path, String relativeToPath) {
        return ConfigurationManager.singleton().getGlobalFactory().doLookup(path, relativeToPath);
    }

    public static String getAlias(String path) {
        return ConfigurationManager.singleton().getGlobalFactory().doGetAlias(path, null);
    }

    public static Object getAlias(String path, String relativeToPath) {
        return ConfigurationManager.singleton().getGlobalFactory().doGetAlias(path, relativeToPath);
    }

    public static void doWork(FactoryWork work) {
        ConfigurationManager.singleton().getGlobalFactory().runWork(work);
    }
